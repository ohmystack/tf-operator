...
func TestFailed(t *testing.T) {
	...
	initializeTFReplicaStatuses(tfJob, tfv1beta2.TFReplicaTypeWorker)
	pod := testutil.NewBasePod("pod", tfJob, t)
	pod.Status.Phase = v1.PodFailed
	updateTFJobReplicaStatuses(tfJob, tfv1beta2.TFReplicaTypeWorker, pod)
	if tfJob.Status.ReplicaStatuses[common.ReplicaType(tfv1beta2.TFReplicaTypeWorker)].Failed != 1 {
		t.Errorf("Failed to set the failed to 1")
	}
	err := ctr.updateStatusSingle(tfJob, tfv1beta2.TFReplicaTypeWorker, 3, false, false)
	if err != nil {
...
func TestStatus(t *testing.T) {
	...
	testCases := []testCase{
		...
		testCase{
			...
			tfJob:                   testutil.NewTFJobWithSuccessPolicy(4, 0, common.SuccessPolicyAllWorkers),
			expectedFailedPS:        0,
			expectedSucceededPS:     0,
			expectedActivePS:        0,
			expectedFailedWorker:    0,
	...
	for i, c := range testCases {
		...
		initializeTFReplicaStatuses(c.tfJob, tfv1beta2.TFReplicaTypeWorker)
		initializeTFReplicaStatuses(c.tfJob, tfv1beta2.TFReplicaTypeChief)
		initializeTFReplicaStatuses(c.tfJob, tfv1beta2.TFReplicaTypePS)

		setStatusForTest(c.tfJob, tfv1beta2.TFReplicaTypePS, c.expectedFailedPS, c.expectedSucceededPS, c.expectedActivePS, t)
		setStatusForTest(c.tfJob, tfv1beta2.TFReplicaTypeWorker, c.expectedFailedWorker, c.expectedSucceededWorker, c.expectedActiveWorker, t)
		setStatusForTest(c.tfJob, tfv1beta2.TFReplicaTypeChief, c.expectedFailedChief, c.expectedSucceededChief, c.expectedActiveChief, t)

		if _, ok := c.tfJob.Spec.TFReplicaSpecs[tfv1beta2.TFReplicaTypeChief]; ok {
			err := ctr.updateStatusSingle(c.tfJob, tfv1beta2.TFReplicaTypeChief, 1, c.restart, c.worker0Completed)
			if err != nil {
				t.Errorf("%s: Expected error %v to be nil", c.description, err)
			}
			if c.tfJob.Spec.TFReplicaSpecs[tfv1beta2.TFReplicaTypeWorker] != nil {
				replicas := c.tfJob.Spec.TFReplicaSpecs[tfv1beta2.TFReplicaTypeWorker].Replicas
				err := ctr.updateStatusSingle(c.tfJob, tfv1beta2.TFReplicaTypeWorker, int(*replicas), c.restart, c.worker0Completed)
				if err != nil {
					t.Errorf("%s: Expected error %v to be nil", c.description, err)
				}
			}
			if c.tfJob.Spec.TFReplicaSpecs[tfv1beta2.TFReplicaTypePS] != nil {
				replicas := c.tfJob.Spec.TFReplicaSpecs[tfv1beta2.TFReplicaTypePS].Replicas
				err := ctr.updateStatusSingle(c.tfJob, tfv1beta2.TFReplicaTypePS, int(*replicas), c.restart, c.worker0Completed)
				if err != nil {
					t.Errorf("%s: Expected error %v to be nil", c.description, err)
				}
			}
...
func setStatusForTest(tfJob *tfv1beta2.TFJob, typ tfv1beta2.TFReplicaType, failed, succeeded, active int32, t *testing.T) {
	pod := testutil.NewBasePod("pod", tfJob, t)
	var i int32
	for i = 0; i < failed; i++ {
		pod.Status.Phase = v1.PodFailed
