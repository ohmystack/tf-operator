...
func (tc *TFController) updateStatusSingle(tfjob *tfv1beta2.TFJob, rtype tfv1beta2.TFReplicaType, replicas int, restart, worker0Completed bool) error {
	...
	if ContainChieforMasterSpec(tfjob) {
		...
		if rtype == tfv1beta2.TFReplicaTypeWorker {
...
func initializeTFReplicaStatuses(tfjob *tfv1beta2.TFJob, rtype tfv1beta2.TFReplicaType) {
	commonType := common.ReplicaType(rtype)
...
func updateTFJobReplicaStatuses(tfjob *tfv1beta2.TFJob, rtype tfv1beta2.TFReplicaType, pod *v1.Pod) {
	commonType := common.ReplicaType(rtype)
