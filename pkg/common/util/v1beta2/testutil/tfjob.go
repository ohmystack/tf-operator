...
func NewTFJobWithCleanPolicy(chief, worker, ps int, policy common.CleanPodPolicy) *tfv1beta2.TFJob {
	if chief == 1 {
...
func NewTFJobWithCleanupJobDelay(chief, worker, ps int, ttl *int32) *tfv1beta2.TFJob {
	if chief == 1 {
		tfJob := NewTFJobWithChief(worker, ps)
...
func NewTFJobWithEvaluator(worker, ps, evaluator int) *tfv1beta2.TFJob {
	tfJob := NewTFJob(worker, ps)
	if evaluator > 0 {
		evaluator := int32(evaluator)
		tfJob.Spec.TFReplicaSpecs[tfv1beta2.TFReplicaTypeEval] = &common.ReplicaSpec{
			Replicas: &evaluator,
			Template: NewTFReplicaSpecTemplate(),
		}
	}
...
func NewTFJob(worker, ps int) *tfv1beta2.TFJob {
	...
	if worker > 0 {
		...
		tfJob.Spec.TFReplicaSpecs[tfv1beta2.TFReplicaTypeWorker] = workerReplicaSpec
	}
	...
	if ps > 0 {
		...
		tfJob.Spec.TFReplicaSpecs[tfv1beta2.TFReplicaTypePS] = psReplicaSpec
	}
	return tfJob
}

func NewTFJobWithNamespace(worker, ps int, ns string) *tfv1beta2.TFJob {
	tfJob := NewTFJob(worker, ps)
	tfJob.Namespace = ns
	...
	return v1.PodTemplateSpec{
		Spec: v1.PodSpec{
			Containers: []v1.Container{
				v1.Container{
					Name:  tfv1beta2.DefaultContainerName,
					Image: TestImageName,
					Args:  []string{"Fake", "Fake"},
					Ports: []v1.ContainerPort{
						v1.ContainerPort{
							Name:          tfv1beta2.DefaultPortName,
							ContainerPort: tfv1beta2.DefaultPort,
						},
					},
				},
			},
...
func SetTFJobCompletionTime(tfJob *tfv1beta2.TFJob) {
